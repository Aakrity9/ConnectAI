...
async function main() {
    ...
    mockVector3[0] = 0.1;
    mockVector3[2] = 0.35;
    const formatVectorSql = (vec) => `[${vec.join(',')}]`;
    console.log('Injecting mock vector embeddings...');
    await prisma.$executeRawUnsafe(`UPDATE "Profile" SET "embedding" = '${formatVectorSql(mockVector1)}'::vector WHERE "id" = '${profile1.id}'`);
    await prisma.$executeRawUnsafe(`UPDATE "Profile" SET "embedding" = '${formatVectorSql(mockVector2)}'::vector WHERE "id" = '${profile2.id}'`);
    await prisma.$executeRawUnsafe(`UPDATE "Profile" SET "embedding" = '${formatVectorSql(mockVector3)}'::vector WHERE "id" = '${profile3.id}'`);
